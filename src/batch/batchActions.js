...
export function getList() {
    const request = axios.get(`${consts.API_URL}/batch?user_email=_${controleValidadeUser['email']}&status=ABERTO`)
    return {
        type: 'BATCH_FETCHED',
        payload: request
    }
...
function submit(values, method) {
    ...
    return dispatch => {
        ...
        axios[method](`${consts.API_URL}/batch/${id}`, values)
            .then(resp => {
...
export function showUpdate(batch) {
    return [
        showTabs('tabUpdate'),
        selectTab('tabUpdate'),
        initialize('batchForm', batch)
    ]
}

export function showDelete(batch) {
    return [
        showTabs('tabDelete'),
        selectTab('tabDelete'),
        initialize('batchForm', batch)
...
export function init() {
    return [
        ...
        selectTab('tabList'),
        getList(),
        initialize('batchForm', INITIAL_VALUES)
    ]
}
